...
extern struct blocking_notifier_head fb_notifier_head;
...
static void sii902x_setup(struct fb_info *fbi)
{
	...
	if(sii902x.color_space == 0) /* Set input format to RGB */
		i2c_smbus_write_byte_data(sii902x.client, 0x09, 0x00);
...
static void det_worker(struct work_struct *work)
{
	...
	// printk(KERN_ERR"status: 0x%02X, sii902x.retries: %d\n", dat, sii902x.retries);
		...
		const struct i2c_device_id *id)
{
	...
	rst_gpio = of_get_named_gpio_flags(np, "hdmi_rst", 0, NULL);
	if (gpio_is_valid(rst_gpio)) {
		err = gpio_request(rst_gpio, "HDMI reset");
		if (!err) {
			gpio_direction_output(rst_gpio, 1);
			gpio_export(rst_gpio, 0);
		} else {
			printk("request HDMI reset[gpio %d] err:%d\n", 
					rst_gpio, err);
		}
	}
