					 ...
					 const struct i2c_device_id *id)
{
    ...
    tsdata->backlight_gpio = devm_gpiod_get_optional(&client->dev,
						    "backlight", GPIOD_OUT_HIGH);
	if (IS_ERR(tsdata->backlight_gpio)) {
		error = PTR_ERR(tsdata->backlight_gpio);
		dev_err(&client->dev,
			"Failed to request GPIO backlight pin, error %d\n", error);
		return error;
	}
    ...
    if (tsdata->backlight_gpio) {
		usleep_range(5000, 6000);
		gpiod_set_value_cansleep(tsdata->backlight_gpio, 0);
		msleep(300);
	}
