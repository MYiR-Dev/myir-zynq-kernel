...
static struct ft5x0x_ts_platform_data *ft5x0x_parse_dt(struct device *dev)
{
	...
	pdata->pwr_en = of_get_named_gpio_flags(np, "pwr_en", 0, NULL);
	// printk("tp pwr_en gpio=%d\n", pdata->pwr_en);

	pdata->bl_en = of_get_named_gpio_flags(np, "bl_en", 0, NULL);
	// printk("tp rebl_enset gpio=%d\n", pdata->bl_en);
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(ts_plat_data->pwr_en)) {
		err = gpio_request(ts_plat_data->pwr_en, "pwr_en");
		if (!err) {
			gpio_direction_output(ts_plat_data->pwr_en, 1);
			gpio_export(ts_plat_data->pwr_en, 0);
			// may be you need some delay here
		} else {
			printk("request pwr_en[gpio %d] err:%d\n", 
					ts_plat_data->pwr_en, err);
		}
	}

	if (gpio_is_valid(ts_plat_data->bl_en)) {
		err = gpio_request(ts_plat_data->bl_en, "bl_en");
		if (!err) {
			gpio_direction_output(ts_plat_data->bl_en, 1);
			gpio_export(ts_plat_data->bl_en, 0);
			// may be you need some delay here
		} else {
			printk("request bl_en[gpio %d] err:%d\n", 
					ts_plat_data->bl_en, err);
		}
	}
	...
	if (!ts_plat_data->polling_mode) {
		err = request_irq(ts_plat_data->irq, ft5x0x_ts_interrupt, 
				0 | IRQF_TRIGGER_FALLING, "ft5x0x_ts", ft5x0x_ts);
		if (err < 0) {
